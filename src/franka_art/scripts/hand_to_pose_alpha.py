...
# Import MediaPipe Tasks for Gesture Recognition
from mediapipe.tasks.python.vision import GestureRecognizer, GestureRecognizerOptions
from mediapipe.tasks.python.core import VisionRunningMode
...
class WristDepthViewer:
    def __init__(self):
        ...
        task_options = GestureRecognizerOptions(
            base_options=mp_core.BaseOptions(model_asset_path='/home/tonappa/Desktop/franka_teleop/src/franka_art/models/gesture_recognizer.task'),
            running_mode=VisionRunningMode.LIVE_STREAM,
            num_hands=1
        )
        self.gesture_recognizer = GestureRecognizer.create_from_options(task_options)
        ...
        self.last_wrist = None  # store last detected wrist (u,v,depth)
    ...
    def reset_reference(self):
        if self.last_wrist:
            u, v, depth = self.last_wrist
            self.ref_u, self.ref_v, self.ref_depth = u, v, depth
            self.init_x, self.init_y, self.init_z = 0.4, 0.0, 0.4
            self.ref_set = True
            rospy.loginfo(
                f"Reference reset: (u,v,depth)=({u},{v},{depth}) -> "
                f"init_pose=({self.init_x},{self.init_y},{self.init_z})"
            )
    ...
    def process_frame(self):
        ...
        if gesture_res.gestures:
            ...
            if gesture == 'Closed_Fist':
                ...
                rospy.loginfo(f"Paused: {self.paused}")
        ...
        if self.ref_set and wrist_u is not None and not self.paused:
            ...
            rospy.logdebug(f"Pose x={x:.3f}, y={y:.3f}, z={z:.3f}")
