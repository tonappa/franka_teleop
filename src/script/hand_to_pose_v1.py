...
# with depth estimation wrist to index finger distance --> robot Z movement
...
class HandToPose:
    def __init__(self):
        ...
        self.scale_x = 0.01  # pixels -> meters
        self.scale_y = 0.01
        self.scale_z = 2.0     # factor to scale depth difference to meters
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if results.multi_hand_landmarks:
                ...
                # MCP indice per orientamento
                mcp = hand_landmarks.landmark[5]

                # calcola vettore direzione polso → MCP indice
                vec = np.array([mcp.x - wrist.x, mcp.y - wrist.y])
                angle = np.arctan2(vec[1], vec[0])  # rotazione nel piano XY

                # Final position
                pos = self.base_pos + np.array([dx, -dy, dz])

                pos[2] = max(0.3, min(pos[2], 0.7)) # limito il un range sicuro, sempre sopra il tavolo
                ...
                # converti in quaternion (rotazione intorno a Z)
